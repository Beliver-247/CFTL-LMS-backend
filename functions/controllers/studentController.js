...
exports.createStudent = async (req, res) => {
  try {
    ...
    // 🛑 Uniqueness check BEFORE the transaction
    const duplicateCheck = await db.collection('students')
      .where('nameFull', '==', body.nameFull)
      .where('dob', '==', Timestamp.fromDate(new Date(dob)))
      .limit(1)
      .get();

    if (!duplicateCheck.empty) {
      return res.status(409).send({
        error: 'A student with the same full name and date of birth already exists.'
      });
    }
    ...
    // ✅ Transaction to create student and increment counter
...
// ✅ GET student by ID
// MODIFIED: No need to fetch course details, enrollment is now separate
